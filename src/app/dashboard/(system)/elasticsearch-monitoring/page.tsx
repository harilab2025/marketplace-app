...
export default function ElasticsearchMonitoringPage() {
    ...
    const [usersIndexStats, setUsersIndexStats] = useState<UsersIndexStats | null>(null);
    const [syncStatus, setSyncStatus] = useState<SyncStatus | null>(null);
    const [usersSyncStatus, setUsersSyncStatus] = useState<UsersSyncStatus | null>(null);
    ...
    const fetchMonitoringData = useCallback(async () => {
        ...
        try {
            const [healthRes, statsResProducts, syncResProducts, statsResUsers, syncResUsers] = await Promise.allSettled([
                getClusterHealth(),
                getProductsIndexStats(),
                checkSyncStatus(),
                getUsersIndexStats(),
            ...
            // Index Stats
            if (statsResProducts.status === 'fulfilled') {
                setIndexStats(statsResProducts.value.data);
            } else {
                console.error('Failed to fetch index stats:', statsResProducts.reason);
                toast.error('Failed to fetch index stats');
            }

            // Sync Status
            if (syncResProducts.status === 'fulfilled') {
                setSyncStatus(syncResProducts.value.data);
            } else {
                console.error('Failed to fetch sync status:', syncResProducts.reason);
                toast.error('Failed to fetch sync status');
            }

            // Users Index Stats
            if (statsResUsers.status === 'fulfilled') {
                console.log('Users Index Stats Response:', statsResUsers.value);
                // Handle both direct data and wrapped response
                const usersStatsData = statsResUsers.value.data || statsResUsers.value;
                setUsersIndexStats(usersStatsData);
            } else {
                console.error('Failed to fetch users index stats:', statsResUsers.reason);
                toast.error('Failed to fetch users index stats');
            }

            // Users Sync Status
            if (syncResUsers.status === 'fulfilled') {
                console.log('Users Sync Status Response:', syncResUsers.value);
                // Handle both direct data and wrapped response
                const usersSyncData = syncResUsers.value.data || syncResUsers.value;
                setUsersSyncStatus(usersSyncData);
            } else {
                console.error('Failed to fetch users sync status:', syncResUsers.reason);
                toast.error('Failed to fetch users sync status');
            }
    ...
    const handleQuickReindex = async () => {
        ...
        try {
            const resultProducts = await reindexProducts(false);
            const dataProducts = resultProducts.data;
            const resultUsers = await reindexUsers(false);
            const dataUsers = resultUsers.data;
            toast.success(
                `Reindexed ${dataProducts.indexed} products in ${dataProducts.duration}${dataProducts.errors > 0 ? ` (${dataProducts.errors} errors)` : ''} and ${dataUsers.indexed} users in ${dataUsers.duration}${dataUsers.errors > 0 ? ` (${dataUsers.errors} errors)` : ''}`
            );
    ...
    // Show loading state
    if (status === 'loading' || (isLoading && !clusterHealth && !indexStats && !syncStatus && !usersIndexStats && !usersSyncStatus)) {
        return (
        ...
        <div className="w-full">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                ...
                <SyncStatusCard
                    ...
                    isLoading={isLoading && !syncStatus && !usersSyncStatus}
                ...
                <div className="lg:col-span-2">
                    <IndexStatsCard
                        stats={indexStats}
                        usersStats={usersIndexStats}
                        isLoading={isLoading && !indexStats && !usersIndexStats}
                    />
                </div>
